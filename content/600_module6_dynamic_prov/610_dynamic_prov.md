...
4. This demo volume has served its purpose, so you can delete it now. The model volume that vLLM is serving from is a completely separate volume and is untouched by this:
...
:::
- This is why the vLLM pod in Module 2 began serving almost immediately. The model data was already sitting on FSx for ONTAP, so there was no multi-gigabyte download to wait for.

**Dynamic provisioning** (demo volume) and **volume import** (model volume) are the two ways Trident connects Kubernetes PVCs to ONTAP storage. You have already seen the import pattern a second time, in Module 4, where the pre-provisioned shared volume holds the finance and IT Ops data.

::::

## Summary

In this section you created a StorageClass (`ontap-nas-sc`) that configures Trident to provision thin-provisioned NFS volumes on your FSx for ONTAP backend, and used a demo PersistentVolumeClaim to watch Trident **dynamically provision** an ONTAP volume, automatically creating the PersistentVolume and the underlying ONTAP volume with no manual PV creation. The Mistral-7B model volume, by contrast, was pre-provisioned and pre-loaded during workshop setup via the volume-import pattern, which is why the vLLM deployment back in Module 2 needed no download wait.
