...
Now create a small PersistentVolumeClaim (PVC) that references the StorageClass. When you apply this PVC, Trident **dynamically provisions** a brand-new ONTAP volume and creates the corresponding PV automatically — no manual PV creation is needed. This is the core pattern you'll use for any application storage on FSx for ONTAP.
...
- **accessModes**: `ReadWriteMany` — allows multiple pods to mount the volume concurrently. In production, you may have 100s of pods mounting the same volume.
...
- **storage**: `1Gi` — small, since this volume is only for demonstration
