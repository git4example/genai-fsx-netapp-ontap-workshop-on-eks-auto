...
The same attack that succeeded moments ago now **fails**. The Kubernetes NetworkPolicy blocks all traffic from the `malicious-agent` pod to the `finance-agent` and `itops-agent` pods. The HTTP request times out because the packets are dropped at the network level, and the finance agent never even sees the request.
